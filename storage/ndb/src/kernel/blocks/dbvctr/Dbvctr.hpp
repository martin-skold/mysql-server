...
#include <signaldata/VctrBound.hpp>
#include <signaldata/VctrContinueB.hpp>
#include <signaldata/VctrMaint.hpp>
...
class Dbvctr : public SimulatedBlock {
  friend class Dbqvctr;
  friend class DbvctrProxy;
  friend struct mt_BuildIndxCtx;
  friend Uint32 Dbvctr_mt_buildIndexFragment_wrapper_C(void *);
 ...
 public:
  Dbvctr(Block_context &ctx, Uint32 instanceNumber = 0, Uint32 blockNo = DBVCTR);
  ~Dbvctr() override;
  ...
  Dblqh *c_lqh;
  Dbacc *c_acc;
  void execVCTR_BOUND_INFO(Signal *signal);
...
#define NullTupLoc TupLoc()
 ...
 private:
   ...
   */
  typedef DataBufferSegment<ScanBoundSegmentSize, RT_DBVCTR_SCAN_BOUND>
      ScanBoundSegment;
  typedef TransientPool<ScanBoundSegment> ScanBoundBuffer_pool;
  static constexpr Uint32 DBVCTR_SCAN_BOUND_TRANSIENT_POOL_INDEX = 2;
  typedef DataBuffer<ScanBoundSegmentSize, ScanBoundBuffer_pool,
                     RT_DBVCTR_SCAN_BOUND>
      ScanBoundBuffer;
  ...
  // ScanLock
  struct ScanLock {
    static constexpr Uint32 TYPE_ID = RT_DBVCTR_SCAN_LOCK;
    Uint32 m_magic;
  ...
  static constexpr Uint32 DBVCTR_SCAN_LOCK_TRANSIENT_POOL_INDEX = 1;
   ...
   */
  struct ScanOp {
    static constexpr Uint32 TYPE_ID = RT_DBVCTR_SCAN_OPERATION;
    Uint32 m_magic;
  ...
  static constexpr Uint32 DBVCTR_SCAN_OPERATION_TRANSIENT_POOL_INDEX = 0;
 ...
 private:
  ...
  // utils
  void readKeyAttrs(VctrCtx &, const Frag &frag, TreeEnt ent, KeyData &keyData,
                    Uint32 count);
  void readKeyAttrs(VctrCtx &, const Frag &frag, TreeEnt ent, Uint32 count,
                    Uint32 *outputBuffer);
  ...
  void execVCTRFRAGREQ(Signal *signal);
  void execVCTR_ADD_ATTRREQ(Signal *signal);
 ...
 public:
   ...
   * DbvctrMaint.cpp
   */
  void execVCTR_MAINT_REQ(Signal *signal);

 private:
  /*
   * DbvctrNode.cpp
   */
  int allocNode(VctrCtx &, NodeHandle &node);
  void freeNode(NodeHandle &node);
  void selectNode(VctrCtx &, NodeHandle &node, TupLoc loc);
  void insertNode(VctrCtx &, NodeHandle &node);
  void deleteNode(NodeHandle &node);
  void freePreallocatedNode(Frag &frag);
  void setNodePref(struct VctrCtx &, NodeHandle &node);
  // node operations
  void nodePushUp(VctrCtx &, NodeHandle &node, unsigned pos, const TreeEnt &ent,
                  Uint32 scanList, Uint32 scanInstance);
  void nodePushUpScans(NodeHandle &node, unsigned pos);
  void nodePopDown(VctrCtx &, NodeHandle &node, unsigned pos, TreeEnt &en,
                   Uint32 *scanList, Uint32 *scanInstance);
  void nodePopDownScans(NodeHandle &node, unsigned pos);
  void nodePushDown(VctrCtx &, NodeHandle &node, unsigned pos, TreeEnt &ent,
                    Uint32 &scanList, Uint32 &scanInstance);
  void nodePushDownScans(NodeHandle &node, unsigned pos);
  void nodePopUp(VctrCtx &, NodeHandle &node, unsigned pos, TreeEnt &ent,
                 Uint32 scanList, Uint32 scanInstance);
  void nodePopUpScans(NodeHandle &node, unsigned pos);
  void nodeSlide(VctrCtx &, NodeHandle &dstNode, NodeHandle &srcNode,
   ...
   */
  // add entry
  void treeAdd(VctrCtx &, Frag &frag, TreePos treePos, TreeEnt ent);
  void treeAddFull(VctrCtx &, Frag &frag, NodeHandle lubNode, unsigned pos,
                   TreeEnt ent);
  void treeAddNode(VctrCtx &, Frag &frag, NodeHandle lubNode, unsigned pos,
                   TreeEnt ent, NodeHandle parentNode, unsigned i);
  void treeAddRebalance(VctrCtx &, Frag &frag, NodeHandle node, unsigned i);
  // remove entry
  ...
  // rotate
  void treeRotateSingle(VctrCtx &, Frag &frag, NodeHandle &node, unsigned i);
  void treeRotateDouble(VctrCtx &, Frag &frag, NodeHandle &node, unsigned i);
   ...
   * DbvctrSearch.cpp
   */
  void findNodeToUpdate(VctrCtx &, Frag &frag, const KeyBoundArray &searchBound,
                        TreeEnt searchEnt, NodeHandle &currNode);
  bool findPosToAdd(VctrCtx &, Frag &frag, const KeyBoundArray &searchBound,
                    TreeEnt searchEnt, NodeHandle &currNode, TreePos &treePos);
  bool findPosToRemove(VctrCtx &, TreeEnt searchEnt, NodeHandle &currNode,
                       TreePos &treePos);
  bool searchToAdd(VctrCtx &, Frag &frag, const KeyBoundArray &searchBound,
                   TreeEnt searchEnt, TreePos &treePos);
  bool searchToRemove(VctrCtx &, Frag &frag, const KeyBoundArray &searchBound,
                      TreeEnt searchEnt, TreePos &treePos);
  ...
  void prepare_scan_bounds(const ScanOp *scanPtrP, const Index *indexPtrP,
                           Dbvctr *vctr_block);
  void prepare_move_scan_ctx(ScanOpPtr scanPtr, Dbvctr *vctr_block);

  int cmpSearchKey(VctrCtx &, const KeyDataC &searchKey,
                   const KeyDataC &entryKey, Uint32 cnt);
  int cmpSearchBound(VctrCtx &, const KeyBoundC &searchBound,
                     const KeyDataC &entryKey, Uint32 cnt);
  ...
  void printNode(struct VctrCtx &, Frag &, NdbOut &out, TupLoc loc,
                 PrintPar &par);
  ...
  struct VctrCtx
      c_ctx;  // Global Vctr context, for everything build MT-index build
 ...
 public:
  ...
  void prepare_build_ctx(VctrCtx &ctx, FragPtr fragPtr);
  void prepare_tup_ptrs(VctrCtx &ctx);
  void prepare_all_tup_ptrs(VctrCtx &ctx);
 ...
 public:
  ...
  Uint32 getDBVCTR() { return m_vctr_block; }
  ...
  ScanOp *getScanOpPtrP(Uint32 scanPtrI, Uint32 scanInstance) {
    ...
    vctr_block = (Dbvctr *)globalData.getBlock(blockNo, instanceNo);
    return vctr_block->getScanOpPtrP(scanPtrI);
  }
};

inline bool Dbvctr::check_freeScanLock(ScanOp &scan) {
...
inline void Dbvctr::release_c_free_scan_lock() {
  if (c_freeScanLock != RNIL) {
    ...
    c_freeScanLock = RNIL;
    checkPoolShrinkNeed(DBVCTR_SCAN_LOCK_TRANSIENT_POOL_INDEX, c_scanLockPool);
  }
}

inline void Dbvctr::checkPoolShrinkNeed(const Uint32 pool_index,
                                       const TransientFastSlotPool &pool) {
...
inline Dbvctr::TupLoc::TupLoc()
    : m_pageId1(RNIL >> 16), m_pageId2(RNIL & 0xFFFF), m_pageOffset(0) {}

inline Dbvctr::TupLoc::TupLoc(Uint32 pageId, Uint16 pageOffset)
    : m_pageId1(pageId >> 16),
      m_pageId2(pageId & 0xFFFF),
      m_pageOffset(pageOffset) {}

inline Uint32 Dbvctr::TupLoc::getPageId() const {
  return (m_pageId1 << 16) | m_pageId2;
}

inline void Dbvctr::TupLoc::setPageId(Uint32 pageId) {
  m_pageId1 = (pageId >> 16);
  m_pageId2 = (pageId & 0xFFFF);
}

inline Uint32 Dbvctr::TupLoc::getPageOffset() const {
  return (Uint32)m_pageOffset;
}

inline void Dbvctr::TupLoc::setPageOffset(Uint32 pageOffset) {
  m_pageOffset = (Uint16)pageOffset;
}

inline bool Dbvctr::TupLoc::operator==(const TupLoc &loc) const {
...
inline Dbvctr::TreeEnt::TreeEnt() : m_tupLoc(), m_tupVersion(0) {}

inline bool Dbvctr::TreeEnt::eqtuple(const TreeEnt ent) const {
  return m_tupLoc == ent.m_tupLoc;
}

inline bool Dbvctr::TreeEnt::eq(const TreeEnt ent) const {
  return m_tupLoc == ent.m_tupLoc && m_tupVersion == ent.m_tupVersion;
}

inline int Dbvctr::TreeEnt::cmp(const TreeEnt ent) const {
...
inline Dbvctr::TreeNode::TreeNode()
    : m_side(2),
      m_balance(0 + 1),
      pad1(0),
      m_occup(0),
...
inline Dbvctr::TreeHead::TreeHead()
    : m_nodeSize(0), m_prefSize(0), m_minOccup(0), m_maxOccup(0), m_root() {}

inline Uint32 *Dbvctr::TreeHead::getPref(TreeNode *node) const {
  Uint32 *ptr = (Uint32 *)node + NodeHeadSize;
  return ptr;
}

inline Dbvctr::TreeEnt *Dbvctr::TreeHead::getEntList(TreeNode *node) const {
...
inline Dbvctr::TreePos::TreePos()
    : m_loc(), m_pos(Uint32(~0)), m_dir(Uint32(~0)) {}

// Dbvctr::DescPage

inline Dbvctr::DescPage::DescPage() : m_nextPage(RNIL), m_numFree(ZNIL) {
...
inline Dbvctr::ScanBound::ScanBound() : m_head(), m_cnt(0), m_side(0) {}

inline Dbvctr::ScanOp::ScanOp()
...
inline Dbvctr::Frag::Frag(ScanOp_pool &scanOpPool)
    : m_tableId(RNIL),
      m_indexId(RNIL),
      m_fragId(ZNIL),
      m_tree(),
...
inline Dbvctr::NodeHandle::NodeHandle(Frag &frag)
    : m_frag(frag), m_loc(), m_node(0) {}

inline Dbvctr::NodeHandle::NodeHandle(const NodeHandle &node)
    : m_frag(node.m_frag), m_loc(node.m_loc), m_node(node.m_node) {}

inline Dbvctr::NodeHandle &Dbvctr::NodeHandle::operator=(const NodeHandle &node) {
...
inline bool Dbvctr::NodeHandle::isNull() { return m_node == 0; }

inline Dbvctr::TupLoc Dbvctr::NodeHandle::getLink(unsigned i) {
  ndbrequire(i <= 2);
  return m_node->m_link[i];
}

inline unsigned Dbvctr::NodeHandle::getChilds() {
  return (m_node->m_link[0] != NullTupLoc) + (m_node->m_link[1] != NullTupLoc);
}

inline unsigned Dbvctr::NodeHandle::getSide() { return m_node->m_side; }

inline unsigned Dbvctr::NodeHandle::getOccup() { return m_node->m_occup; }

inline int Dbvctr::NodeHandle::getBalance() {
  return (int)m_node->m_balance - 1;
}

inline bool Dbvctr::NodeHandle::isNodeScanList() {
  return m_node->m_nodeScanPtrI != RNIL;
}

inline void Dbvctr::NodeHandle::getNodeScan(Uint32 &scanPtrI, Uint32 &instance) {
  scanPtrI = m_node->m_nodeScanPtrI;
  instance = m_node->m_nodeScanInstance;
}

inline void Dbvctr::NodeHandle::setLink(unsigned i, TupLoc loc) {
  if (likely(i <= 2)) {
    m_node->m_link[i] = loc;
  } else {
    ndbabort();
...
inline Uint32 Dbvctr::get_instance_from_scan_instance(Uint32 scanInstance) {
  return (scanInstance >> 16);
}

inline Uint32 Dbvctr::get_block_from_scan_instance(Uint32 scanInstance) {
  return (scanInstance & 0xFFFF);
}

inline Uint32 Dbvctr::get_my_scan_instance() {
  if (m_is_query_block) {
    return DBQVCTR + (instance() << 16);
  } else {
    return DBVCTR + (instance() << 16);
  }
}

inline void Dbvctr::NodeHandle::setSide(unsigned i) {
  if (likely(i <= 2)) {
    m_node->m_side = i;
  } else {
    ndbabort();
...
inline void Dbvctr::NodeHandle::setOccup(unsigned n) {
  TreeHead &tree = m_frag.m_tree;
  ndbrequire(n <= tree.m_maxOccup);
  m_node->m_occup = n;
}

inline void Dbvctr::NodeHandle::setBalance(int b) {
  ndbrequire(abs(b) <= 1);
  m_node->m_balance = (unsigned)(b + 1);
}

inline void Dbvctr::NodeHandle::setNodeScan(Uint32 scanPtrI, Uint32 instance) {
  m_node->m_nodeScanPtrI = scanPtrI;
  m_node->m_nodeScanInstance = instance;
}

inline Uint32 *Dbvctr::NodeHandle::getPref() {
  TreeHead &tree = m_frag.m_tree;
  return tree.getPref(m_node);
}

inline Dbvctr::TreeEnt Dbvctr::NodeHandle::getEnt(unsigned pos) {
  TreeHead &tree = m_frag.m_tree;
...
inline Dbvctr::StatOp::Value::Value() {
  m_rir = 0;
  Uint32 i;
  for (i = 0; i < MaxKeyCount; i++) m_unq[i] = 0;
}

inline Dbvctr::StatOp::StatOp(const Index &index)
    : m_scanOpPtrI(RNIL),
      m_saveSize(0),
      m_saveScale(0),
      m_batchSize(0),
...
#ifdef VM_TRACE
inline Dbvctr::PrintPar::PrintPar()
    :  // caller fills in
      m_path(),
      m_side(255),
      m_parent(),
...
inline Uint32 Dbvctr::getDescSize(const Index &index) {
  return DescHeadSize + index.m_numAttrs * KeyTypeSize +
         index.m_numAttrs * AttributeHeaderSize;
}

inline Dbvctr::DescHead &Dbvctr::getDescHead(const Index &index) {
  DescPagePtr pagePtr;
...
inline Dbvctr::KeyType *Dbvctr::getKeyTypes(DescHead &descHead) {
  Uint32 *ptr = reinterpret_cast<Uint32 *>(&descHead);
  ptr += DescHeadSize;
  return reinterpret_cast<KeyType *>(ptr);
}

inline const Dbvctr::KeyType *Dbvctr::getKeyTypes(const DescHead &descHead) {
  const Uint32 *ptr = reinterpret_cast<const Uint32 *>(&descHead);
  ptr += DescHeadSize;
  return reinterpret_cast<const KeyType *>(ptr);
}

inline AttributeHeader *Dbvctr::getKeyAttrs(DescHead &descHead) {
  Uint32 *ptr = reinterpret_cast<Uint32 *>(&descHead);
  ptr += DescHeadSize;
...
inline const AttributeHeader *Dbvctr::getKeyAttrs(const DescHead &descHead) {
  const Uint32 *ptr = reinterpret_cast<const Uint32 *>(&descHead);
  ptr += DescHeadSize;
  ptr += descHead.m_numAttrs * KeyTypeSize;
...
inline void Dbvctr::getTupAddr(const Frag &frag, TreeEnt ent, Uint32 &lkey1,
                              Uint32 &lkey2) {
...
inline unsigned Dbvctr::min(unsigned x, unsigned y) { return x < y ? x : y; }

inline unsigned Dbvctr::max(unsigned x, unsigned y) { return x > y ? x : y; }
 ...
 */
inline int Dbvctr::cmpSearchKey(VctrCtx &ctx, const KeyDataC &searchKey,
                               const KeyDataC &entryKey, Uint32 cnt) {
  ...
  if (debugFlags & DebugMaint) {
    vctrDebugOut << "cmpSearchKey: ret:" << ret;
    vctrDebugOut << " search:"
...
inline int Dbvctr::cmpSearchBound(VctrCtx &ctx, const KeyBoundC &searchBound,
                                 const KeyDataC &entryKey, Uint32 cnt) {
  // compare cnt attributes from each
  Uint32 num_eq;
  ...
  if (debugFlags & DebugScan) {
    vctrDebugOut << "cmpSearchBound: res:" << ret;
    vctrDebugOut << " search:"
...
inline void Dbvctr::prepare_all_tup_ptrs(VctrCtx &ctx) {
  c_tup->get_all_tup_ptrs(
      ...
      &ctx.tupRealTablePtr, ctx.attrDataOffset, ctx.vctrFixHeaderSize);
}

inline void Dbvctr::relinkScan(Uint32 line) {
