 ...
 *   scanned, this is the fragment record in VCTR.
 *
 * c_ctx.indexPtr
 ...
 *   in VCTR to avoid having to include Dbtup.hpp in VCTR.
...
void Dbvctr::prepare_scan_ctx(Uint32 scanPtrI) {
  jamDebug();
  FragPtr fragPtr;
  ScanOpPtr scanPtr;
  IndexPtr indexPtr;
 ...
 */
void Dbvctr::prepare_move_scan_ctx(ScanOpPtr scanPtr, Dbvctr *vctr_block) {
  Index *indexPtrP = c_ctx.indexPtr.p;
  c_ctx.scanPtr = scanPtr;
  prepare_scan_bounds(scanPtr.p, indexPtrP, vctr_block);
}

/**
 * This method is called either from building of an index
 * or when updating an index from execVCTR_MAINT_REQ. It sets
 ...
 */
void Dbvctr::prepare_build_ctx(VctrCtx &ctx, FragPtr fragPtr) {
  IndexPtr indexPtr;
  ctx.fragPtr = fragPtr;
  indexPtr.i = fragPtr.p->m_indexId;
  c_indexPool.getPtr(indexPtr);
 ...
 */
void Dbvctr::prepare_scan_bounds(const ScanOp *scanPtrP, const Index *indexPtrP,
                                Dbvctr *vctr_block) {
  jamDebug();
  ...
  if (likely(scanBound.m_cnt != 0)) {
    ...
    vctr_block->unpackBound(c_ctx.c_nextKey, scanBound, searchBound);
...
void Dbvctr::execACC_CHECK_SCAN(Signal *signal) {
  jamEntryDebug();
  ...
  if (debugFlags & DebugScan) {
    vctrDebugOut << "ACC_CHECK_SCAN scan " << scanPtr.i << " " << scan << endl;
  }
#endif
 ...
 */
void Dbvctr::execACC_SCANREQ(Signal *signal) {
  jamEntry();
  ...
  do {
    ...
    if (unlikely(indexPtr.p->m_state != Index::Online)) {
      ...
      if (debugFlags & (DebugMeta | DebugScan)) {
        vctrDebugOut << "Index dropping at ACC_SCANREQ " << indexPtr.i << " "
                    << *indexPtr.p << endl;
      }
#endif
    ...
    if (unlikely(isStatScan)) {
      ...
      // rest of StatOp is initialized in execVCTR_BOUND_INFO
#ifdef VM_TRACE
      if (debugFlags & DebugStat) {
        vctrDebugOut << "Seize stat op" << endl;
      }
#endif
    }
#ifdef VM_TRACE
    if (debugFlags & DebugScan) {
      vctrDebugOut << "Seize scan " << scanPtr.i << " " << *scanPtr.p << endl;
    }
#endif
    // conf
    AccScanConf *const conf = (AccScanConf *)signal->getDataPtrSend();
 ...
 */
void Dbvctr::execVCTR_BOUND_INFO(Signal *signal) {
  jamEntry();
  // get records
  VctrBoundInfo *const req = (VctrBoundInfo *)signal->getDataPtrSend();
  ScanOpPtr scanPtr = c_ctx.scanPtr;
  ScanOp &scan = *scanPtr.p;
  ...
  for (unsigned idir = 0; idir <= 1; idir++) {
    ...
    while (offset + 2 <= boundLen) {
      ...
      if (unlikely(type > 4)) {
        jam();
        scan.m_errorCode = VctrBoundInfo::InvalidAttrInfo;
        req->errorCode = scan.m_errorCode;
        return;
      }
      Uint32 type2 = type;
      ...
      if ((type2 & 0x2) == (idir << 1)) {
        ...
        if (unlikely(b.type2 != -1)) {
          jam();
          scan.m_errorCode = VctrBoundInfo::InvalidBounds;
          req->errorCode = scan.m_errorCode;
          return;
        }
        b.type2 = (int)type2;
    ...
    for (i = 0; i < maxAttrId; i++) {
      ...
      if (unlikely(searchBoundData.add_poai(&data[b.offset], &len) == -1 ||
                   b.bytes != len)) {
        jam();
        scan.m_errorCode = VctrBoundInfo::InvalidCharFormat;
        req->errorCode = scan.m_errorCode;
        return;
      }
    }
    ...
    // save data words in segmented memory
    {
      ...
      if (unlikely(!ok)) {
        jam();
        scan.m_errorCode = VctrBoundInfo::OutOfBuffers;
        req->errorCode = scan.m_errorCode;
        return;
      }
    }
...
void Dbvctr::execNEXT_SCANREQ(Signal *signal) {
...
#ifdef VM_TRACE
  if (debugFlags & DebugScan) {
    vctrDebugOut << "NEXT_SCANREQ scan " << c_ctx.scanPtr.i << " " << scan
                << endl;
  }
#endif
...
void Dbvctr::continue_scan(Signal *signal, ScanOpPtr scanPtr, Frag &frag,
...
#ifdef VM_TRACE
    if (debugFlags & (DebugMeta | DebugScan)) {
      vctrDebugOut << "Index dropping at execACC_CHECK_SCAN " << scanPtr.i << " "
                  << *scanPtr.p << endl;
    }
#endif
      ...
      switch (lockReq->returnCode) {
        case AccLockReq::Success: {
          ...
          if (debugFlags & (DebugScan | DebugLock)) {
            vctrDebugOut << "Lock immediate scan " << scanPtr.i << " " << scan
                        << endl;
          }
#endif
          break;
...
#ifdef VM_TRACE
          if (debugFlags & (DebugScan | DebugLock)) {
            vctrDebugOut << "Lock wait scan " << scanPtr.i << " " << scan
                        << endl;
          }
#endif
          // LQH will wake us up
 ...
 */
void Dbvctr::execACCKEYCONF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
...
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "Lock obtained scan " << scanPtr.i << " " << scan << endl;
  }
#endif
 ...
 */
void Dbvctr::execACCKEYREF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
  scanPtr.i = signal->theData[0];
  ndbrequire(c_scanOpPool.getValidPtr(scanPtr));
...
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "Lock refused scan " << scanPtr.i << " " << scan << endl;
  }
#endif
  ndbrequire(scan.m_lockwait && scan.m_accLockOp != RNIL);
  scan.m_lockwait = false;
 ...
 */
void Dbvctr::execACC_ABORTCONF(Signal *signal) {
  jamEntry();
  ScanOpPtr scanPtr;
  scanPtr.i = signal->theData[0];
  ndbrequire(c_scanOpPool.getValidPtr(scanPtr));
...
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "ACC_ABORTCONF scan " << scanPtr.i << " " << scan << endl;
  }
#endif
 ...
 */
void Dbvctr::scanFirst(ScanOpPtr scanPtr, Frag &frag, const Index &index) {
  ScanOp &scan = *scanPtr.p;
  // scan direction 0, 1
 ...
 */
void Dbvctr::scanFind(ScanOpPtr scanPtr, Frag &frag) {
  ScanOp &scan = *scanPtr.p;
 ...
 */
Uint32 Dbvctr::scanNext(ScanOpPtr scanPtr, bool fromMaintReq, Frag &frag) {
  ScanOp &scan = *scanPtr.p;
...
bool Dbvctr::checkScanInstance(Uint32 scanInstance) {
  // General scanInstance handling rules :
  //                  Instances handled :
  //  - VCTR block   : VCTR or (QVCTR iff Q threads)
  //  - QVCTR thread : (My QVCTR id only iff Q threads)
  const bool i_am_vctr = !m_is_query_block;
  const bool scanInstance_is_vctr =
      (get_block_from_scan_instance(scanInstance) == DBVCTR);

  /* Basic sanity */
  ndbrequire((i_am_vctr && scanInstance_is_vctr) ||
             (globalData.ndbMtQueryThreads > 0));

  if (i_am_vctr) {
    /* VCTR */
    if (scanInstance_is_vctr) {
      if (scanInstance != m_my_scan_instance) {
        /* VCTR should not deal with some other VCTR's scanInstances */
        ndbrequire(false);
      } else {
        /* ok - VCTR instance dealing with own scanInstance */
      }
    } else {
      /* VCTR instance dealing with QVCTR scan - ok */
    }
  } else {
    /**
     * QVCTR instance should only ever deal with its own scanInstances,
     * not other QVCTR's, or VCTR's.
     */
...
void Dbvctr::relinkScan(ScanOp &scan, Uint32 scanInstance, Frag &frag,
   ...
   *
   * When this method is called from a VCTR index reorganisation we
 ...
 * invalid data.
 */
inline bool Dbvctr::scanCheck(ScanOp &scan, TreeEnt ent) {
  jamDebug();
 ...
 */
bool Dbvctr::scanVisible(ScanOp &scan, TreeEnt ent) {
 ...
 * send a REF.
 */
void Dbvctr::scanClose(Signal *signal, ScanOpPtr scanPtr) {
  ScanOp &scan = *scanPtr.p;
  ndbrequire(!scan.m_lockwait && scan.m_accLockOp == RNIL);
...
void Dbvctr::abortAccLockOps(Signal *signal, ScanOpPtr scanPtr) {
  ScanOp &scan = *scanPtr.p;
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "Abort locks in scan " << scanPtr.i << " " << scan << endl;
  }
#endif
...
void Dbvctr::addAccLockOp(ScanOpPtr scanPtr, Uint32 accLockOp) {
  ScanOp &scan = *scanPtr.p;
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "Add lock " << hex << accLockOp << dec << " to scan "
                << scanPtr.i << " " << scan << endl;
  }
#endif
  Local_ScanLock_fifo list(c_scanLockPool, scan.m_accLockOps);
...
void Dbvctr::removeAccLockOp(ScanOpPtr scanPtr, Uint32 accLockOp) {
  ScanOp &scan = *scanPtr.p;
#ifdef VM_TRACE
  if (debugFlags & (DebugScan | DebugLock)) {
    vctrDebugOut << "Remove lock " << hex << accLockOp << dec << " from scan "
                << scanPtr.i << " " << scan << endl;
  }
#endif
  Local_ScanLock_fifo list(c_scanLockPool, scan.m_accLockOps);
 ...
 */
void Dbvctr::releaseScanOp(ScanOpPtr &scanPtr) {
#ifdef VM_TRACE
  if (debugFlags & DebugScan) {
    vctrDebugOut << "Release scan " << scanPtr.i << " " << *scanPtr.p << endl;
  }
#endif
  ...
  checkPoolShrinkNeed(DBVCTR_SCAN_BOUND_TRANSIENT_POOL_INDEX, c_scanBoundPool);
  ...
  checkPoolShrinkNeed(DBVCTR_SCAN_OPERATION_TRANSIENT_POOL_INDEX, c_scanOpPool);
}
