 ...
 */
int Dbvctr::allocNode(VctrCtx &ctx, NodeHandle &node) {
  if (ERROR_INSERTED(12007)) {
    jam();
    CLEAR_ERROR_INSERT_VALUE;
    return VctrMaintReq::NoMemError;
  }
  ...
  if (likely(errorCode == 0)) {
    ...
    switch (errorCode) {
      case 827:
        ...
        errorCode = VctrMaintReq::NoMemError;
        break;
      case 921:
        thrjam(ctx.jamBuffer);
        errorCode = VctrMaintReq::NoTransMemError;
        break;
    }
  }
  return errorCode;
 ...
 */
void Dbvctr::freeNode(NodeHandle &node) {
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::selectNode(VctrCtx &ctx, NodeHandle &node, TupLoc loc) {
  ndbrequire(loc != NullTupLoc);
  Uint32 pageId = loc.getPageId();
  Uint32 pageOffset = loc.getPageOffset();
  Uint32 *node32 = 0;
  c_tup->tuxGetNode(ctx.attrDataOffset, ctx.vctrFixHeaderSize, pageId,
                    pageOffset, node32);
  node.m_loc = loc;
  node.m_node = reinterpret_cast<TreeNode *>(node32);
  ndbrequire(node.m_loc != NullTupLoc && node.m_node != 0);
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::insertNode(VctrCtx &ctx, NodeHandle &node) {
 ...
 */
void Dbvctr::deleteNode(NodeHandle &node) {
  Frag &frag = node.m_frag;
 ...
 */
void Dbvctr::freePreallocatedNode(Frag &frag) {
  if (frag.m_freeLoc != NullTupLoc) {
    jam();
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::setNodePref(VctrCtx &ctx, NodeHandle &node) {
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::nodePushUp(VctrCtx &ctx, NodeHandle &node, unsigned pos,
 ...
 * index.
 */
void Dbvctr::nodePushUpScans(NodeHandle &node, unsigned pos) {
  const unsigned occup = node.getOccup();
  ScanOpPtr scanPtr;
  Uint32 scanInstance;
  ...
  do {
    ...
    if (scanPos.m_pos >= pos) {
      ...
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Fix scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "At pushUp pos=" << pos << " " << node << endl;
      }
#endif
      scanPos.m_pos++;
    }
 ...
 * else moved forward.
 */
void Dbvctr::nodePopDown(VctrCtx &ctx, NodeHandle &node, unsigned pos,
...
void Dbvctr::nodePopDownScans(NodeHandle &node, unsigned pos) {
  const unsigned occup = node.getOccup();
  ScanOpPtr scanPtr;
  Uint32 scanInstance;
  node.getNodeScan(scanPtr.i, scanInstance);
  do {
    ...
    if (scanPos.m_pos > pos) {
      ...
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Fix scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "At popDown pos=" << pos << " " << node << endl;
      }
#endif
      scanPos.m_pos--;
    }
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::nodePushDown(VctrCtx &ctx, NodeHandle &node, unsigned pos,
                         TreeEnt &ent, Uint32 &scanList, Uint32 &scanInstance) {
  Frag &frag = node.m_frag;
  TreeHead &tree = frag.m_tree;
  const unsigned occup = node.getOccup();
 ...
 */
void Dbvctr::nodePushDownScans(NodeHandle &node, unsigned pos) {
  const unsigned occup = node.getOccup();
  ScanOpPtr scanPtr;
  Uint32 scanInstance;
  node.getNodeScan(scanPtr.i, scanInstance);
  do {
    ...
    if (scanPos.m_pos <= pos) {
      ...
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Fix scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "At pushDown pos=" << pos << " " << node << endl;
      }
#endif
      scanPos.m_pos--;
    }
 ...
 */
void Dbvctr::nodePopUp(VctrCtx &ctx, NodeHandle &node, unsigned pos, TreeEnt &ent,
...
void Dbvctr::nodePopUpScans(NodeHandle &node, unsigned pos) {
  const unsigned occup = node.getOccup();
  ScanOpPtr scanPtr;
  Uint32 scanInstance;
  node.getNodeScan(scanPtr.i, scanInstance);
  do {
    ...
    if (scanPos.m_pos < pos) {
      ...
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Fix scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "At popUp pos=" << pos << " " << node << endl;
      }
#endif
      scanPos.m_pos++;
    }
 ...
 */
void Dbvctr::nodeSlide(VctrCtx &ctx, NodeHandle &dstNode, NodeHandle &srcNode,
 ...
 */
void Dbvctr::addScanList(NodeHandle &node, unsigned pos, Uint32 scanList,
                        Uint32 scanInstance) {
  ScanOpPtr scanPtr;
  scanPtr.i = scanList;
  do {
...
#ifdef VM_TRACE
    if (debugFlags & DebugScan) {
      vctrDebugOut << "Add scan " << scanPtr.i << " " << *scanPtr.p << endl;
      vctrDebugOut << "To pos= " << pos << " " << node << endl;
    }
#endif
 ...
 * building ordered indexes.
 */
void Dbvctr::removeScanList(NodeHandle &node, unsigned pos, Uint32 &scanList,
                           Uint32 &scanInstance) {
  ScanOpPtr scanPtr;
...
#ifdef VM_TRACE
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Remove scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "From pos=" << pos << " " << node << endl;
      }
#endif
 ...
 * method scanNext().
 */
void Dbvctr::moveScanList(NodeHandle &node, unsigned pos) {
  ScanOpPtr scanPtr;
  Uint32 scanInstance;
  node.getNodeScan(scanPtr.i, scanInstance);
  do {
    ...
    if (scanPos.m_pos == pos) {
...
#ifdef VM_TRACE
      if (debugFlags & DebugScan) {
        vctrDebugOut << "Move scan " << scanPtr.i << " " << *scanPtr.p << endl;
        vctrDebugOut << "At pos=" << pos << " " << node << endl;
      }
#endif
      /**
       ...
       *
       * This method is called during an update of the VCTR index, thus we
       * are guaranteed that there are no other concurrent activity on the
       * VCTR index at the moment. So we don't really need to lock the
       * index fragment.
       *
       * When we arrive here we need to use a scan record from another
       * DBVCTR/DBQVCTR instance. For the most part the scan record is standing
      ...
      Dbvctr *vctr_block = (Dbvctr *)globalData.getBlock(blockNo, instanceNo);
      prepare_move_scan_ctx(scanPtr, vctr_block);
 ...
 */
void Dbvctr::linkScan(NodeHandle &node, ScanOpPtr scanPtr, Uint32 scanInstance) {
 ...
 * ordered indexes.
 */
void Dbvctr::unlinkScan(NodeHandle &node, ScanOpPtr scanPtr,
                       Uint32 scanInstance) {
  ScanOpPtr currPtr;
  Uint32 loc_scanInstance;
 ...
 */
bool Dbvctr::islinkScan(NodeHandle &node, ScanOpPtr scanPtr,
                       Uint32 scanInstance) {
  ScanOpPtr currPtr;
  Uint32 loc_scanInstance;
  node.getNodeScan(currPtr.i, loc_scanInstance);
...
void Dbvctr::NodeHandle::progError(int line, int cause, const char *file,
                                  const char *check) {
  char buf[500];
  /*Add the check to the log message only if default value of ""
    is over-written. */
  if (native_strcasecmp(check, "") == 0)
    BaseString::snprintf(buf, sizeof(buf), "Dbvctr::NodeHandle: assert failed");
  else
    BaseString::snprintf(buf, sizeof(buf),
                         "Dbvctr::NodeHandle: assert %.400s failed", check);
