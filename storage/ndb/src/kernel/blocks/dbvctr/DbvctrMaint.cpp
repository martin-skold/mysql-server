...
void Dbvctr::execVCTR_MAINT_REQ(Signal *signal) {
  jamEntryDebug();
  VctrMaintReq *const sig = (VctrMaintReq *)signal->getDataPtrSend();
  ...
  if (unlikely(!(indexPtr.p->m_state == Index::Online ||
    ...
    if (debugFlags & DebugMaint) {
      ...
      vctrDebugOut << "opInfo=" << hex << sig->opInfo;
      vctrDebugOut << " tableId=" << dec << sig->tableId;
      vctrDebugOut << " indexId=" << dec << sig->indexId;
      vctrDebugOut << " fragId=" << dec << sig->fragId;
      vctrDebugOut << " tupLoc=" << tupLoc;
      vctrDebugOut << " tupVersion=" << dec << sig->tupVersion;
      vctrDebugOut << " -- ignored at ISP=" << dec << c_internalStartPhase;
      vctrDebugOut << " TOS=" << dec << c_typeOfStart;
      vctrDebugOut << endl;
    }
#endif
    sig->errorCode = 0;
  ...
  VctrMaintReq reqCopy = *sig;
  VctrMaintReq *const req = &reqCopy;
  ...
  if (debugFlags & DebugMaint) {
    ...
    vctrDebugOut << "opCode=" << dec << opCode;
    vctrDebugOut << " opFlag=" << dec << opFlag;
    vctrDebugOut << " tableId=" << dec << req->tableId;
    vctrDebugOut << " indexId=" << dec << req->indexId;
    vctrDebugOut << " fragId=" << dec << req->fragId;
    vctrDebugOut << " entry=" << ent;
    vctrDebugOut << endl;
  }
#endif
  // do the operation
  ...
  bool ok;
  switch (opCode) {
    case VctrMaintReq::OpAdd:
      jamDebug();
      ok = searchToAdd(c_ctx, frag, *searchBound, ent, treePos);
#ifdef VM_TRACE
      if (debugFlags & DebugMaint) {
        vctrDebugOut << treePos << (!ok ? " - error" : "") << endl;
      }
#endif
      if (unlikely(!ok)) {
        jam();
        ...
        if (indexPtr.p->m_state == Index::Online) {
          jam();
          req->errorCode = VctrMaintReq::SearchError;
        }
        break;
      }
      /*
