...
void Dbvctr::execCREATE_TAB_REQ(Signal *signal) {
  jamEntry();
  ...
  do {
    ...
    if (debugFlags & DebugMeta) {
      vctrDebugOut << "Seize frag op " << fragOpPtr.i << " " << *fragOpPtr.p
                  << endl;
    }
#endif
...
void Dbvctr::execVCTR_ADD_ATTRREQ(Signal *signal) {
  jamEntry();
  ...
  do {
...
#ifdef VM_TRACE
    if (debugFlags & DebugMeta) {
      vctrDebugOut << "attr " << attrId << " " << keyType << endl;
    }
#endif
    if (csNumber != 0) {
      unsigned err;
...
#ifdef VM_TRACE
      if (debugFlags & DebugMeta) {
        vctrDebugOut << "Release frag op " << fragOpPtr.i << " " << *fragOpPtr.p
                    << endl;
      }
#endif
      c_fragOpPool.release(fragOpPtr);
    ...
    sendSignal(fragOpPtr.p->m_userRef, GSN_VCTR_ADD_ATTRCONF, signal,
  ...
  ref->errorCode = errorCode;
  sendSignal(fragOpPtr.p->m_userRef, GSN_VCTR_ADD_ATTRREF, signal,
             TuxAddAttrRef::SignalLength, JBB);
#ifdef VM_TRACE
  if (debugFlags & DebugMeta) {
    vctrDebugOut << "Release on attr error frag op " << fragOpPtr.i << " "
                << *fragOpPtr.p << endl;
  }
#endif
  // let DICT drop the unfinished index
...
void Dbvctr::execVCTRFRAGREQ(Signal *signal) {
  jamEntry();
  ...
  do {
...
#ifdef VM_TRACE
    if (debugFlags & DebugMeta) {
      vctrDebugOut << "Add frag " << fragPtr.i << " " << *fragPtr.p << endl;
    }
#endif
    // error inserts
    ...
    if (debugFlags & DebugMeta) {
      if (fragNo == 0) {
        vctrDebugOut << "Index id=" << indexPtr.i;
        vctrDebugOut << " nodeSize=" << tree.m_nodeSize;
        vctrDebugOut << " headSize=" << NodeHeadSize;
        vctrDebugOut << " prefSize=" << tree.m_prefSize;
        vctrDebugOut << " entrySize=" << TreeEntSize;
        vctrDebugOut << " minOccup=" << tree.m_minOccup;
        vctrDebugOut << " maxOccup=" << tree.m_maxOccup;
        vctrDebugOut << endl;
      }
    }
#endif
    ...
    sendSignal(req->userRef, GSN_VCTRFRAGCONF, signal, TuxFragConf::SignalLength,
               JBB);
    return;
  } while (0);
  ...
  ref->errorCode = errorCode;
  sendSignal(req->userRef, GSN_VCTRFRAGREF, signal, TuxFragRef::SignalLength,
             JBB);
 ...
 */
void Dbvctr::abortAddFragOp(Signal *signal) {
  FragOpPtr fragOpPtr;
  IndexPtr indexPtr;
...
#ifdef VM_TRACE
  if (debugFlags & DebugMeta) {
    vctrDebugOut << "Release on abort frag op " << fragOpPtr.i << " "
                << *fragOpPtr.p << endl;
  }
#endif
  c_fragOpPool.release(fragOpPtr);
 ...
 */
void Dbvctr::execALTER_INDX_IMPL_REQ(Signal *signal) {
  jamEntry();
  ...
  if (!(refToBlock(req->senderRef) == DBDICT) &&
      !(isNdbMt() && refToMain(req->senderRef) == DBVCTR &&
...
void Dbvctr::execDROP_TAB_REQ(Signal *signal) {
  jamEntry();
...
#ifdef VM_TRACE
  if (debugFlags & DebugMeta) {
    vctrDebugOut << "Drop index " << indexPtr.i << " " << *indexPtr.p << endl;
  }
#endif
...
void Dbvctr::dropIndex(Signal *signal, IndexPtr indexPtr, Uint32 senderRef,
                      Uint32 senderData) {
  jam();
  /*
...
bool Dbvctr::allocDescEnt(IndexPtr indexPtr) {
  jam();
...
void Dbvctr::freeDescEnt(IndexPtr indexPtr) {
  DescPagePtr pagePtr;
...
void Dbvctr::execDROP_FRAG_REQ(Signal *signal) {
