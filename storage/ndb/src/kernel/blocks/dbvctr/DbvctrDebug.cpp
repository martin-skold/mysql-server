...
void Dbvctr::execDBINFO_SCANREQ(Signal *signal) {
 ...
 */
void Dbvctr::execDUMP_STATE_ORD(Signal *signal) {
  jamEntry();
#ifdef VM_TRACE
  if (signal->theData[0] == DumpStateOrd::VctrLogToFile) {
    unsigned flag = signal->theData[1];
    const char *const vctrlog = "vctr.log";
    FILE *slFile = globalSignalLoggers.getOutputStream();
    if (flag <= 3) {
      if (debugFile != 0) {
        if (debugFile != slFile) fclose(debugFile);
        debugFile = 0;
        vctrDebugOut = *new NdbOut(*new NullOutputStream());
      }
      if (flag == 1) debugFile = fopen(vctrlog, "w");
      if (flag == 2) debugFile = fopen(vctrlog, "a");
      if (flag == 3) debugFile = slFile;
      if (debugFile != 0)
        vctrDebugOut = *new NdbOut(*new FileOutputStream(debugFile));
    }
    return;
  }
  if (signal->theData[0] == DumpStateOrd::VctrSetLogFlags) {
    debugFlags = signal->theData[1];
    return;
  }
  if (signal->theData[0] == DumpStateOrd::VctrMetaDataJunk) {
    abort();
  }
#endif
  ...
  if (signal->theData[0] == DumpStateOrd::VctrSetTransientPoolMaxSize) {
    jam();
  ...
  if (signal->theData[0] == DumpStateOrd::VctrResetTransientPoolMaxSize) {
    jam();
...
void Dbvctr::printTree(Signal *signal, Frag &frag, NdbOut &out) {
  ...
  if (!par.m_ok) {
    if (debugFile == 0) {
      ...
      if (debugFile != 0) {
        printTree(signal, frag, vctrDebugOut);
      }
    }
    ndbabort();
  }
...
void Dbvctr::printNode(VctrCtx &ctx, Frag &frag, NdbOut &out, TupLoc loc,
                      PrintPar &par) {
  if (loc == NullTupLoc) {
    par.m_depth = 0;
    return;
...
#ifdef dbvctr_totally_groks_t_trees
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::TupLoc &loc) {
  if (loc == Dbvctr::NullTupLoc) {
    out << "null";
  } else {
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::TreeEnt &ent) {
  out << ent.m_tupLoc;
  out << "-" << dec << ent.m_tupVersion;
  return out;
}

NdbOut &operator<<(NdbOut &out, const Dbvctr::TreeNode &node) {
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::TreeHead &tree) {
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::TreePos &pos) {
  out << "[TreePos " << hex << &pos;
  out << " [loc " << pos.m_loc << "]";
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::ScanOp &scan) {
  Dbvctr *vctr = (Dbvctr *)globalData.getBlock(DBVCTR);
  out << "[ScanOp " << hex << &scan;
  ...
  if (globalData.isNdbMtLqh)  // TODO
    return out;
  {
    const Dbvctr::ScanLock_fifo::Head &head = scan.m_accLockOps;
    Dbvctr::ConstLocal_ScanLock_fifo list(vctr->c_scanLockPool, head);
    Dbvctr::ScanLockPtr lockPtr;
    list.first(lockPtr);
  ...
  for (unsigned i = 0; i <= 1; i++) {
    const Dbvctr::ScanBound scanBound = scan.m_scanBound[i];
    const Dbvctr::Index &index = *vctr->c_indexPool.getPtr(scan.m_indexId);
    Dbvctr::KeyDataC keyBoundData(index.m_keySpec, true);
    Dbvctr::KeyBoundC keyBound(keyBoundData);
    vctr->unpackBound(vctr->c_ctx.c_searchKey, scanBound, keyBound);
    out << " [scanBound " << dec << i;
    out << " "
        << keyBound.print(vctr->c_ctx.c_debugBuffer, Dbvctr::DebugBufferBytes);
    out << "]";
  }
  return out;
}

NdbOut &operator<<(NdbOut &out, const Dbvctr::Index &index) {
  Dbvctr *vctr = (Dbvctr *)globalData.getBlock(DBVCTR);
  out << "[Index " << hex << &index;
  ...
  for (unsigned i = 0; i < index.m_numFrags; i++) {
    out << " [frag " << dec << i << " ";
    const Dbvctr::Frag &frag = *vctr->c_fragPool.getPtr(index.m_fragPtrI[i]);
    out << frag;
    out << "]";
  }
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::Frag &frag) {
  out << "[Frag " << hex << &frag;
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::FragOp &fragOp) {
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::NodeHandle &node) {
  const Dbvctr::Frag &frag = node.m_frag;
  const Dbvctr::TreeHead &tree = frag.m_tree;
  out << "[NodeHandle " << hex << &node;
  out << " [loc " << node.m_loc << "]";
  out << " [node " << *node.m_node << "]";
  const Uint32 *data;
  out << " [pref";
  data = (const Uint32 *)node.m_node + Dbvctr::NodeHeadSize;
  for (unsigned j = 0; j < tree.m_prefSize; j++) out << " " << hex << data[j];
  out << "]";
  out << " [entList";
  unsigned numpos = node.m_node->m_occup;
  data = (const Uint32 *)node.m_node + Dbvctr::NodeHeadSize + tree.m_prefSize;
  const Dbvctr::TreeEnt *entList = (const Dbvctr::TreeEnt *)data;
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::StatOp &stat) {
  out << "[StatOp " << hex << &stat;
...
NdbOut &operator<<(NdbOut &out, const Dbvctr::StatMon &mon) {
  out << "[StatMon";
