...
#define DBVCTR_STAT_CPP
#include <math.h>
#include "Dbvctr.hpp"

#define JAM_FILE_ID 545

// debug note: uses new-style debug macro "D" unlike rest of DBVCTR
// there is no filtering feature (yet) like "DebugStat"

void Dbvctr::execREAD_PSEUDO_REQ(Uint32 scanPtrI, Uint32 attrId, Uint32 *out,
                                Uint32 out_words) {
  jamEntry();
  ScanOpPtr scanPtr;
  scanPtr.i = scanPtrI;
 ...
 */
void Dbvctr::statRecordsInRange(ScanOpPtr scanPtr, Uint32 *out,
                               Uint32 out_words) {
 ...
 */
Uint32 Dbvctr::getEntriesBeforeOrAfter(Frag &frag, TreePos pos, unsigned idir) {
 ...
 */
unsigned Dbvctr::getPathToNode(NodeHandle node, Uint16 *path) {
...
// windows has no log2
static double vctr_log2(double x) { return ::log(x) / ::log((double)2.0); }

int Dbvctr::statScanInit(StatOpPtr statPtr, const Uint32 *data, Uint32 len,
                        Uint32 *usedLen) {
  ScanOpPtr scanPtr;
  ...
  while (offset + 1 <= len) {
    ...
    switch (type) {
      case VctrBoundInfo::StatSaveSize:
        jam();
        stat.m_saveSize = value;
        break;
      case VctrBoundInfo::StatSaveScale:
        jam();
  ...
  *usedLen = offset;
  ...
  // compute batch size - see wl4124.txt
  {
    ...
    if (g < 1.0) g = 1.0;
    double h = 1 + 0.01 * b * vctr_log2(g);  // scale factor
...
int Dbvctr::statScanAddRow(StatOpPtr statPtr, TreeEnt ent) {
  StatOp &stat = *statPtr.p;
  ScanOpPtr scanPtr;
  scanPtr.i = stat.m_scanOpPtrI;
  ndbrequire(c_scanOpPool.getValidPtr(scanPtr));
...
void Dbvctr::statScanReadKey(StatOpPtr statPtr, Uint32 *out, Uint32 out_words) {
  StatOp &stat = *statPtr.p;
  int ret;
...
void Dbvctr::statScanReadValue(StatOpPtr statPtr, Uint32 *out,
                              Uint32 out_words) {
  StatOp &stat = *statPtr.p;
  int ret;
  Uint32 len_out;
...
void Dbvctr::execINDEX_STAT_REP(Signal *signal) {
  jamEntry();
...
void Dbvctr::execINDEX_STAT_IMPL_REQ(Signal *signal) {
  jamEntry();
...
void Dbvctr::statMonStart(Signal *signal, StatMon &mon) {
...
void Dbvctr::statMonStop(Signal *signal, StatMon &mon) {
  const IndexStatImplReq *req = &mon.m_req;
  Index &index = *c_indexPool.getPtr(req->indexId);
  D("statMonStop" << V(mon));
...
void Dbvctr::statMonConf(Signal *signal, StatMon &mon) {
  const IndexStatImplReq *req = &mon.m_req;
  D("statMonConf" << V(mon));
...
void Dbvctr::statMonSendContinueB(Signal *signal) {
  StatMon &mon = c_statMon;
  D("statMonSendContinueB" << V(mon));

  signal->theData[0] = VctrContinueB::StatMon;
  signal->theData[1] = mon.m_loopIndexId;
  sendSignalWithDelay(reference(), GSN_CONTINUEB, signal, mon.m_loopDelay, 2);
}

void Dbvctr::statMonExecContinueB(Signal *signal) {
  StatMon &mon = c_statMon;
...
void Dbvctr::statMonCheck(Signal *signal, StatMon &mon) {
    ...
    if (index.m_statLoadTime == 0) {
      ...
      if (count <= 1) {
        ...
        const double scaleFactor =
            1 + 0.01 * triggerScale * vctr_log2(fragEntryCount);
...
void Dbvctr::statMonRep(Signal *signal, StatMon &mon) {
  const Index &index = *c_indexPool.getPtr(mon.m_loopIndexId);
  const Frag &frag = *c_fragPool.getPtr(index.m_statFragPtrI);
  D("statMonRep" << V(mon));
