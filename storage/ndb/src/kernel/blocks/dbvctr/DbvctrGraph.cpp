 ...
 */
void Dbvctr::treeAdd(VctrCtx &ctx, Frag &frag, TreePos treePos, TreeEnt ent) {
 ...
 */
void Dbvctr::treeAddFull(VctrCtx &ctx, Frag &frag, NodeHandle lubNode,
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::treeAddNode(VctrCtx &ctx, Frag &frag, NodeHandle lubNode,
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::treeAddRebalance(VctrCtx &ctx, Frag &frag, NodeHandle node,
                             unsigned i) {
  while (true) {
 ...
 */
void Dbvctr::treeRemove(Frag &frag, TreePos treePos) {
  TreeHead &tree = frag.m_tree;
  unsigned pos = treePos.m_pos;
  NodeHandle node(frag);
 ...
 */
void Dbvctr::treeRemoveInner(Frag &frag, NodeHandle lubNode, unsigned pos) {
  TreeEnt ent;
  // find g.l.b node
  NodeHandle glbNode(frag);
  TupLoc loc = lubNode.getLink(0);
 ...
 */
void Dbvctr::treeRemoveSemi(Frag &frag, NodeHandle semiNode, unsigned i) {
  TreeHead &tree = frag.m_tree;
 ...
 * do nothing.
 */
void Dbvctr::treeRemoveLeaf(Frag &frag, NodeHandle leafNode) {
  TreeHead &tree = frag.m_tree;
 ...
 * Remove empty leaf.
 */
void Dbvctr::treeRemoveNode(Frag &frag, NodeHandle leafNode) {
  TreeHead &tree = frag.m_tree;
 ...
 */
void Dbvctr::treeRemoveRebalance(Frag &frag, NodeHandle node, unsigned i) {
  while (true) {
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::treeRotateSingle(VctrCtx &ctx, Frag &frag, NodeHandle &node,
                             unsigned i) {
  ndbrequire(i <= 1);
  /*
 ...
 * Can be called from MT-build of ordered indexes.
 */
void Dbvctr::treeRotateDouble(VctrCtx &ctx, Frag &frag, NodeHandle &node,
                             unsigned i) {
  TreeHead &tree = frag.m_tree;
