...
#include "DbvctrProxy.hpp"
#include "../dblqh/DblqhCommon.hpp"
#include "Dbvctr.hpp"

#define JAM_FILE_ID 548

DbvctrProxy::DbvctrProxy(Block_context &ctx) : LocalProxy(DBVCTR, ctx) {
  // GSN_ALTER_INDX_IMPL_REQ
  addRecSignal(GSN_ALTER_INDX_IMPL_REQ, &DbvctrProxy::execALTER_INDX_IMPL_REQ);
  addRecSignal(GSN_ALTER_INDX_IMPL_CONF, &DbvctrProxy::execALTER_INDX_IMPL_CONF);
  addRecSignal(GSN_ALTER_INDX_IMPL_REF, &DbvctrProxy::execALTER_INDX_IMPL_REF);

  // GSN_INDEX_STAT_IMPL_REQ
  addRecSignal(GSN_INDEX_STAT_IMPL_REQ, &DbvctrProxy::execINDEX_STAT_IMPL_REQ);
  addRecSignal(GSN_INDEX_STAT_IMPL_CONF, &DbvctrProxy::execINDEX_STAT_IMPL_CONF);
  addRecSignal(GSN_INDEX_STAT_IMPL_REF, &DbvctrProxy::execINDEX_STAT_IMPL_REF);

  // GSN_INDEX_STAT_REP
  addRecSignal(GSN_INDEX_STAT_REP, &DbvctrProxy::execINDEX_STAT_REP);
}

DbvctrProxy::~DbvctrProxy() {}

SimulatedBlock *DbvctrProxy::newWorker(Uint32 instanceNo) {
  return new Dbvctr(m_ctx, instanceNo);
}

// GSN_ALTER_INDX_IMPL_REQ

void DbvctrProxy::execALTER_INDX_IMPL_REQ(Signal *signal) {
  jam();
...
void DbvctrProxy::sendALTER_INDX_IMPL_REQ(Signal *signal, Uint32 ssId,
...
void DbvctrProxy::execALTER_INDX_IMPL_CONF(Signal *signal) {
  jam();
...
void DbvctrProxy::execALTER_INDX_IMPL_REF(Signal *signal) {
  jam();
...
void DbvctrProxy::sendALTER_INDX_IMPL_CONF(Signal *signal, Uint32 ssId) {
  jam();
  Ss_ALTER_INDX_IMPL_REQ &ss = ssFind<Ss_ALTER_INDX_IMPL_REQ>(ssId);
...
void DbvctrProxy::execINDEX_STAT_IMPL_REQ(Signal *signal) {
  jamEntry();
...
void DbvctrProxy::sendINDEX_STAT_IMPL_REQ(Signal *signal, Uint32 ssId,
                                         SectionHandle *) {
  jam();
...
void DbvctrProxy::execINDEX_STAT_IMPL_CONF(Signal *signal) {
  jamEntry();
...
void DbvctrProxy::execINDEX_STAT_IMPL_REF(Signal *signal) {
  jamEntry();
...
void DbvctrProxy::sendINDEX_STAT_IMPL_CONF(Signal *signal, Uint32 ssId) {
  jam();
  Ss_INDEX_STAT_IMPL_REQ &ss = ssFind<Ss_INDEX_STAT_IMPL_REQ>(ssId);
  BlockReference dictRef = ss.m_req.senderRef;
...
void DbvctrProxy::execINDEX_STAT_REP(Signal *signal) {
  jamEntry();
  ...
  sendSignal(numberToRef(DBVCTR, instanceNo, getOwnNodeId()), GSN_INDEX_STAT_REP,
             signal, signal->getLength(), JBB);
}

BLOCK_FUNCTIONS(DbvctrProxy)
